...
def like(self, request, pk, *args, **kwargs):
    like = Like_Dislike.objects.get(pk=pk)
    
    is_dislike = False
    
    for dislike in like.dislikes.all():
        if dislike == request.user:
            is_dislike = True
            break
    
    if is_dislike:
        like.dislikes.remove(request.user)
    
    is_like = False
    
    for like in like.likes.all():
...
def dislike(self, request, pk, *args, **kwargs):
    ...
    if not is_dislike:
        like.dislikes.add(request.user)
